...
void ABall::Tick(float DeltaTime)
{
	...
	if (ProjectileMovement->Velocity.X > 1000.0f) {
		SM_Ball->AddImpulse(FVector(240.0f, 0.0f, 230.0f), FName(),true);
		ProjectileMovement->Velocity.X = ProjectileMovement->Velocity.X - 1000.0f;
	}
	else {
		ProjectileMovement->Velocity.X = ProjectileMovement->Velocity.X + 1.0f;
	}
}
...
void ABall::Aceleracion()
{
	ProjectileMovement->Velocity.X = 1000.0f;
	ProjectileMovement->Velocity.Y = 1000.0f;
}
